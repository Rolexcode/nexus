    ...
    <main className={styles.page}>
      ...
      <section className={styles.directory}>
        <div className={styles.sectionHeading}>
          ...
          <Link href="/map" className={styles.mapButton}><Navigation size={16} /> Open full campus map</Link>
        </div>

        <Link href="/map" className={styles.searchShortcut}>
          <Search size={20} />
          <span>Search buildings, food, printing or services</span>
          <ArrowRight size={17} />
        </Link>

        <div className={styles.placeList}>
          {featuredPlaces.map((place) => (
            <Link href={`/map?place=${encodeURIComponent(place.id)}`} className={styles.place} key={place.id}>
